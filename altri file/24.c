/*
LUNGHEZZA MASSIMA STRINGA NOME: 20
LUNGHEZZA MASSIMA CODA DI PICKUP: 2 153
LUNGHEZZA MASSIMA EXIT QUEUE: 2 153
LUNGHEZZA MASSIMA WAIT QUEUE: 74 558
NUMERO MASSIMO DI INGREDIENTI DIVERSI PRESENTI IN MAGAZZINO ALLO STESSO TEMPO: 30
NUMERO MASSIMO DI RICETTE PRESENTI ALLO STESSO TEMPO: 25 000
NUMERO MASSIMO DI INGREDIENTI IN UNA RICETTA: 10
*/
...
int main()
{
    ...
    while(!feof(stdin))
    {
        ...
        if(!feof(stdin))
        {
            ...
            else if(command[2]=='d') ordine(book, storage, &exit_queue, &wait_queue, &exit_tail, &wait_tail, time);
            else
            {
...
void rimuovi_ricetta(recipe* book[], order* exit_queue, order* wait_queue)
{
    ...
    else
    {
        ...
        {
            if(ptr_order->name==ptr_recipe->name) flag=1;
...
void check_wait_queue(recipe* book[], lot* storage[], order** exit_queue, order** wait_queue, order** exit_tail, order** wait_tail, int time)
{
    ...
    while(ptr_order!=NULL)
    {
        ...
        ptr_recipe=book[key];
        while(ptr_recipe->name!=ptr_order->name) ptr_recipe=ptr_recipe->next;
...
int check_enough_ingredients(recipe* ptr_recipe, lot* storage[], int quantity, int time)
{
    ...
    for(i=0; ptr_recipe->array[i].address!=NULL && flag==0; i++)
    {
        ...
        ptr_lot=storage[key];
        while(ptr_lot!=NULL && ptr_lot->name!=ptr_recipe->array[i].address->name) ptr_lot=ptr_lot->next;
        ...
        else
        {
            ...
            if(ptr_lot->head==NULL && ptr_lot->references==0)
            {
            ...
            if(ptr_lot==NULL) flag=1;
            else if(ptr_lot->weight<quantity*ptr_recipe->array[i].weight) flag=1;
        }
    }
    return flag;
}
...
void make_order(recipe* ptr_recipe, lot* storage[], int quantity)
{
    ...
    for(i=0; ptr_recipe->array[i].address!=NULL; i++)
    {
        key=hash(ptr_recipe->array[i].address->name);
        ptr_lot=storage[key];
        while(ptr_lot->name!=ptr_recipe->array[i].address->name) ptr_lot=ptr_lot->next;
