...
int check_enough_ingredients(recipe* ptr_recipe, lot* storage[], int quantity) //FATTO
{
    int flag=0, key;
...
void controlla_scaduti(lot* storage[], int time) //FATTO
{
    int i;
    lot* ptr_lot, *del_lot;
    expiry* del;
    ...
    for(i=0; i<A_LEN; i++)
    {
        ...
        while(ptr_lot!=NULL)
        {
            ...
            {
                del=ptr_lot->head;
                ptr_lot->weight=ptr_lot->weight-(ptr_lot->head)->weight;
                ptr_lot->head=(ptr_lot->head)->next;
                free(del);
            }
            if(ptr_lot->head==NULL)
            {
                if(ptr_lot==storage[i])
                {
                    storage[i]=storage[i]->next;
                    if(storage[i]!=NULL) storage[i]->prev=NULL;
                    free(ptr_lot);
                    ptr_lot=storage[i];
                }
                else
